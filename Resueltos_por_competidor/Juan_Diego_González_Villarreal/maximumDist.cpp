...
struct Node {
    int destino, peso,product;

    Node(int _destino, int _peso, int _product) : destino(_destino), peso(_peso), product(_product) {
    }

    Node() : destino(-1), peso(-1), product(-1) {
    }
};
...
int main() {
    ...
    for (int i = 0; i < E; i++) {
        cin >> origen >> destino;
        ady[origen].push_back(Node{destino, 1,p[destino-1]}); //dirigido
        ady[destino].push_back(Node{origen,1,p[origen-1]});// no dirigido
    }
    dijkstra(0, V);
}