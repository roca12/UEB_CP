 ...
 ***ID: 10007
 ***Juez: UVA
 ***Tipo: Bignum, Catalan Numbers, binary trees
 **********************************************/

//Catalan Number* n! -> number of trees + its permutations
#include <bits/stdc++.h>
#include <cstdlib>
using namespace std;
const int MAXD = 800, DIG = 9, BASE = 1000000000;
...
void Catalan(){
    ...
    int j;
    for(int i = 1; i <= 300; i++){
        dp[i]=(bignum(i)*4-2)*dp[i-1]/(bignum(i)+1);
    }
}
