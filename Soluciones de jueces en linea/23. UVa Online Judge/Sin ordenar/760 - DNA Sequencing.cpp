...
int main(){
    ...
    while( gets( str ) ){
        ...
        if( b ) printf("\n");
        b = true;
        int len = strlen( str ) , i , j , l = len , sub = 0;
        str[ len++ ] = '.';

        for( i = 0 ; s[ i ] ; ++i ) str[ len++ ] = s[ i ];
        str[ len ] = '\0';
        ...
        string ant = "-1";
        for( i = 1 ; i <= len ; ++i ){
            if( ( ( pos[ i ] >= l && pos[ i - 1 ] < l ) ||  ( pos[ i ] < l && pos[ i - 1 ] >= l ) ) && sub > 0 && LCP[ i ] == sub ){
                string aux = "";
                for( j = 0 ; j < LCP[ i ] ; ++j ) aux += str[ pos[ i ] + j ];
                if( aux != ant ) printf("%s\n" , aux.c_str() );
                ant = aux;
            }
        }
        if( sub == 0 ) puts("No common sequence.");
        gets( s );
    }
    return 0;
}
